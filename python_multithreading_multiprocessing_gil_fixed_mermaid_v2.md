...
> **Diagram Fix:** The Mermaid diagrams for **Sequential Execution vs Multithreading** and **Why join Is Needed** were simplified to avoid Mermaid parser issues caused by special characters such as parentheses and quotes inside node labels.
...
```mermaid
flowchart LR
    A[Main Thread] --> B[Run hello function fully]
    B --> C[Run hi function after hello]

    D[Main Thread] --> E[Thread 1 runs hello]
    D --> F[Thread 2 runs hi]
    E --> G[Both tasks make progress concurrently]
    F --> G
```
...
flowchart LR
    ...
    M --> T2[t2 running]
    T1 --> J[Main thread waits using join]
    T2 --> J
    J --> B[Print Bye]
```
